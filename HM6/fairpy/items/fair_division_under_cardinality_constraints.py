...
def greedy_round_robin(category:str, items:set, agents:list, agents_evaluation:dict) -> dict:#-> bundles.Bundle
    ...
    index =0
    allocation, M = {a:set() for a in agents} ,{k for k in agents_evaluation[agents[index]]["{c}".format(c=category)].keys()}
    while len(M) != 0:
        ...
        for i in range(len(set(M))):
            ...
            evaluation_arr = dict({key:value for key, value in agents_evaluation[agent_name]["{c}".format(c=category)].items() if key in M})
